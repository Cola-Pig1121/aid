...
def check_api_keys(config):
    ...
    if modelscope_key:
        ...
        print("[错误] 未找到 API 密钥!")
        print("请在 config.toml 中设置 modelscope.api_key 或 openrouter.api_key")
        print("- ModelScope: https://www.modelscope.cn/my/myaccesstoken")
        print("- OpenRouter: https://openrouter.ai/keys")
        return None


def create_components(provider: str, config):
    """创建LLM客户端、工具和Agent"""
    ...
    location_tool = LocationTool(config=config)
    ...
    if config.tavily_api_key:
        tools.append(SearchTool(api_key=config.tavily_api_key))
        tools.append(HospitalSearchTool(api_key=config.tavily_api_key))
...
============================================================
    ...
    # 检查API密钥并获取提供商
    provider = args.provider or check_api_keys(config)
    if not provider:
        sys.exit(1)
